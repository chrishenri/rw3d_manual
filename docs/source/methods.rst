...
.. warning::
    ...
    **Concentration versus Total Density**
    ...
    If ``read_concentration_file`` is the specified injection type (``type_inj``), the code will internally convert the concentration values to total densities. 
    ...
    If concentration values has to be estimated from the outputs, the user should convert local particle densities reflecting total densities (from plume snapshots) to concentrations.
